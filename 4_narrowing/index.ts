// type guard (verificação pelo typeof)
type sumType = string | number
const sum = (a: sumType, b: sumType): sumType => {
...
const operations = (arr: number[], operator?: string | undefined): number | string => {
  if(operator) {
    if(operator === 'sum') {
      const result: number = arr.reduce((curr, acc) => curr + acc) 
      return result
    } 
...
function review(name: string, stars: number | boolean) {
  if(typeof stars === "number") {
    if(stars > 3) return `${name} gostou muito desse filme, deu ${stars} estrelas.`
    else if(stars < 3) return `${name} não gostou desse filme, deu ${stars} estrela(s).`
    else if(stars === 3) return `${name} deu ${stars} estrelas. Filme mediano, igual minha vida.`
